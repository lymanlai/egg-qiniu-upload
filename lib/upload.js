...
class Qiniu {
  ...
  async uploadWithFileName (path = '', realname = '') {
    const options = {
      scope: `${this.option.bucket}:${realname}`,
      expires: 24 * 60 * 60
    }
    const putPolicy = new qiniu.rs.PutPolicy(options)
    const uploadToken = putPolicy.uploadToken(this.mac)
  ...
  async info (key) {
    ...
    return new Promise((resolved, reject) => {
      bucketManager.stat(this.option.bucket, key, function (
        err,
        respBody,
        respInfo
      ) {
...
const mount = app => {
  ...
  assert(
    config.ak && config.sk,
    `[egg-qiniu-upload] ak: ${config.ak}, sk: ${config.sk}`
  )
  assert(
    config.bucket && config.baseUrl,
    `[egg-qiniu-upload] bucket: ${config.bucket}, baseUrl: ${config.baseUrl}`
  )
